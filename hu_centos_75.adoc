...
summary: Instale Linux Host Utilities en un host CentOS 7.5 para administrar operaciones de protocolo FCP e iSCSI con LUN de ONTAP . 
---
= Configurar CentOS 7.5 para FCP e iSCSI para almacenamiento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
El software de utilidades de host Linux proporciona herramientas de gestión y diagnóstico para hosts Linux que se conectan al almacenamiento de ONTAP. Cuando instala las utilidades de host de Linux en un host CentOS 7.5, puede usarlas para ayudarlo a administrar las operaciones de protocolo FCP e iSCSI con LUN de ONTAP.
...
Puede utilizar rutas múltiples con CentOS 7.5 para administrar LUN de ONTAP.
...
El host CentOS 7.5 con almacenamiento ONTAP tiene los siguientes problemas conocidos:
