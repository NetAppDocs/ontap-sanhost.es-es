...
summary: Cómo configurar el host NVMe-oF para Rocky Linux 10.0 con almacenamiento ONTAP 
---
= Configurar Rocky Linux 10.0 para NVMe-oF con almacenamiento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Aprenda a configurar hosts NVMe over Fabrics (NVMe-oF) para Rocky Linux 10.0. Para obtener más información sobre asistencia y funciones, consulte link:nvme-rockylinux-supported-features.html["Compatibilidad y características de Rocky Linux ONTAP"].

*NVMe-oF con Rocky Linux 10.0 tiene las siguientes limitaciones conocidas:*

* La compatibilidad con múltiples rutas DM para NVMe-oF no es compatible.
...
.Pasos
. Instalar Rocky Linux 10.0 en el servidor.  Una vez completada la instalación, verifique que esté ejecutando el kernel Rocky Linux 10.0 requerido:
+
[source, cli]
----
uname -r
...
A partir de ONTAP 9.12.1, se admite la autenticación segura en banda a través de NVMe/TCP entre un host Rocky Linux 10.0 y un controlador ONTAP .
...
cat /etc/nvme/hostnqn
----
. Genere la clave dhchap para el host Rocky Linux 10.0.
+
