...
summary: Describe cómo utilizar Red Hat Enterprise Linux 7.3 con ONTAP 
---
= Utilice Red Hat Enterprise Linux 7,3 con ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Puede utilizar los ajustes de configuración del host SAN de ONTAP para configurar Red Hat Enterprise Linux 7,3 con ONTAP como destino.
...
Para Red Hat Enterprise Linux (RHEL) 7.3 debe existir el archivo /etc/multipath.conf, pero no es necesario realizar cambios específicos en el archivo. RHEL 7.3 se ha compilado con todas las configuraciones necesarias para reconocer y gestionar correctamente las LUN de ONTAP.
...
El sistema operativo RHEL 7.3 se compila para reconocer las LUN de ONTAP y establecer automáticamente todos los parámetros de configuración correctamente para la configuración tanto de ASA como de terceros.
...
No existen problemas conocidos para la versión de RHEL 7,3 con ONTAP.
