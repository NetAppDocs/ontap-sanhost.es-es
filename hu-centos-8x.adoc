...
summary: Instale las utilidades de host de Linux en un host CentOS 8.x para administrar las operaciones de protocolo FCP e iSCSI con LUN de ONTAP . 
---
= Configure CentOS 8.x para FCP e iSCSI para el almacenamiento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
El software Linux Host Utilities proporciona herramientas de administración y diagnóstico para los hosts Linux que están conectados al almacenamiento ONTAP . Cuando instale las utilidades de host de Linux en un host CentOS 8.x, podrá utilizarlas para ayudarle a gestionar las operaciones de los protocolos FCP e iSCSI con LUN de ONTAP .
...
Puedes utilizar multipathing con CentOS 8.x para gestionar LUN de ONTAP .
...
Para los problemas conocidos del kernel compatible con CentOS Red Hat, consulta el link:hu-rhel-8x.html#step-7-review-the-known-issues["problemas conocidos"] para la serie Red Hat Enterprise Linux (RHEL) 8.x.
...
* Obtenga más información sobre la virtualización de CentOS Linux (KVM)
+
CentOS Linux puede servir como host KVM. Esto le permite ejecutar varias máquinas virtuales en un solo servidor físico utilizando la tecnología de máquina virtual basada en kernel de Linux (KVM). El host KVM no requiere configuraciones de host explícitas para los LUN de ONTAP .
