...
---
= Configuración del host NVMe/FC para RHEL 8.3 con ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
Para RHEL 8.3, la función multivía de NVMe en el kernel sigue deshabilitada de forma predeterminada. Por lo tanto, debe habilitarla manualmente. Los pasos para hacerlo se proporcionan en la siguiente sección, "activación de NVMe/FC en RHEL 8.3".
...
+
Si va a actualizar de RHEL 8.2 a RHEL 8.3 mediante `yum update/upgrade`, el `/etc/nvme/host*` es posible que se pierdan archivos. Para evitar la pérdida de archivos, haga lo siguiente:
...
----
. En el host RHEL 8.3, compruebe la cadena de NQN del host en /etc/nvme/hostnqn y verifique que coincide con la cadena de NQN del host para el subsistema correspondiente en la cabina de ONTAP.
+
[listing]
----
...
TIP: Si las cadenas del host NQN no coinciden, utilice `vserver modify` Comando para actualizar la cadena del NQN del host en el subsistema de cabina de ONTAP correspondiente para que coincida con la cadena NQM del host desde /etc/nvme/hostnqn en el host.

. Reinicie el host.
. Actualice el `enable_foreign` ajuste _(opcional)_.
+

NOTE: Si piensa ejecutar el tráfico NVMe y SCSI en el mismo host coexistente de RHEL 8.3, le recomendamos que utilice la multivía de NVMe en el kernel para espacios de nombres de ONTAP y dm-multipath para LUN de ONTAP, respectivamente. También debe incluir una lista negra de los espacios de nombres ONTAP en dm-multipath para evitar que dm-multipath reclamen estos dispositivos de espacio de nombres. Para ello, añada la `enable_foreign` la configuración en /etc/multipath.conf, tal como se muestra a continuación.
