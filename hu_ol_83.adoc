...
summary: Describe cómo usar Oracle Linux 8.3 con ONTAP 
---
= Utilice Oracle Linux 8,3 con ONTAP
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
:toclevels: 1P
...
[role="lead"]
Es posible usar las opciones de configuración del host SAN de ONTAP para configurar Oracle Linux 8,3 con ONTAP como destino.
...
Para Oracle Linux (OL) 8.3 debe existir el archivo /etc/multipath.conf, pero no es necesario realizar cambios específicos en el archivo. OL 8.3 se compila con todas las configuraciones necesarias para reconocer y gestionar correctamente las LUN de ONTAP.
...
El sistema operativo Oracle Linux 8.3 se compila para reconocer las LUN ONTAP y establecer automáticamente todos los parámetros de configuración correctamente para la configuración ASA y no ASA.
...
NOTE: Para configurar Red Hat Enterprise Kernel de Oracle Linux 8.3 (RHCK), utilice link:hu_rhel_83.html#recommended-settings["configuración recomendada"] Para Red Hat Enterprise Linux (RHEL) 8.3.



== Problemas conocidos

No existen problemas conocidos para la versión Oracle Linux 8,3 con ONTAP.


NOTE: Para ver los problemas conocidos de Oracle Linux (kernel compatible con Red Hat), consulte link:hu_rhel_83.html#known-problems-and-limitations["problemas conocidos"] En la documentación correspondiente de la versión de Red Hat Enterprise Linux.
