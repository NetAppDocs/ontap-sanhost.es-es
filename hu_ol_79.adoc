...
---
= Uso de Oracle Linux 7.9 con NetApp ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
Puede utilizar el `multipath -ll` Comando para verificar la configuración de las LUN de ONTAP. Las siguientes secciones proporcionan ejemplos de salidas multivía para una LUN asignada a personas no ASA y ASA.
...
El sistema operativo Oracle Linux 7,9 se compila para reconocer los LUN de ONTAP y definir automáticamente todos los parámetros de configuración correctamente.
...
NOTE: Para configurar Oracle Linux 7.9 RedHat Enterprise Kernel (RHCK), utilice link:hu_rhel_79.html#recommended-settings["configuración recomendada"] Para Red Hat Enterprise Linux (RHEL) 7.9.
...
| link:https://mysupport.netapp.com/NOW/cgi-bin/bol?Type=Detail&Display=1328944["1328943"^] | Retrasos de I/o observados durante la conmutación por error del almacenamiento con el host LPe32002 (32 G) de Emulex | Es posible que las operaciones de I/o no puedan continuar durante las rutas activas existentes durante las operaciones de recuperación tras fallos de almacenamiento en el kernel de Oracle Linux 7.8 (5.4.17-2011.1.2.el8uek.x86_64) con HBA 32 G de QLogic LPe32002. Si el progreso de I/o se detiene debido a un fallo en la continuidad de las rutas activas existentes durante la conmutación al respaldo del almacenamiento, es posible que la operación de I/o no se reanude lo que provoca un retraso de I/O. La operación de I/o se reanuda solo una vez que se completa la operación de devolución del nodo de respaldo de almacenamiento. | link:https://bugzilla.oracle.com/bugzilla/show_bug.cgi?id=17319["17319"^] 
|===