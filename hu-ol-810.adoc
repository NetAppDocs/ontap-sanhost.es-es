...
NOTE: Para configurar Oracle Linux 8,10 RedHat Enterprise Kernel, utilice link:hu_rhel_810.html#recommended-settings["configuración recomendada"]para Red Hat Enterprise Linux (RHEL) 8,10.



== Configuración de KVM

También puede utilizar <<recommended_settings_ol_810,configuración recomendada>>para configurar una máquina virtual basada en kernel (KVM). No se necesitan cambios para configurar un KVM porque la LUN está asignada al hipervisor.
