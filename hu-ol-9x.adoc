...
summary: Instale las utilidades de host de Linux en un host Oracle Linux 9.x para administrar las operaciones de protocolo FCP e iSCSI con LUN de ONTAP . 
---
= Configurar Oracle Linux 9.x para FCP e iSCSI con almacenamiento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
El software Linux Host Utilities proporciona herramientas de administración y diagnóstico para los hosts Linux que están conectados al almacenamiento ONTAP . Cuando instale las utilidades de host de Linux en un host Oracle Linux 9.x, podrá utilizarlas para ayudarle a gestionar las operaciones de protocolo FCP e iSCSI con LUN de ONTAP .
...
Puede utilizar rutas múltiples con Oracle Linux 9.x para administrar LUN de ONTAP .
...
Oracle Linux 9.3, 9.2, 9.1 y 9.0 con almacenamiento ONTAP presenta el siguiente problema conocido:
...
| https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1508554["1508554"^] | La utilidad de SAN LUN con HBA de Emulex necesita enlaces simbólicos de paquetes de la biblioteca | Al ejecutar el comando de la CLI de Linux Host Utilities, "sanlun fcp show adapter -v", en un host SAN, el comando falla con un mensaje de error que indica que no se encuentran las dependencias de biblioteca necesarias para la detección de un adaptador de bus de host (HBA): ---- [root@hostname ~]# sanlun fcp show adapter -v No se pudo encontrar la biblioteca /usr/lib64/libHBAAPI.so. Asegúrese de que el paquete que instala la biblioteca esté instalado y cargado. ---- 
|===
