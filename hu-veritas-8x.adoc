...
summary: Utilice Linux Host Utilities con Veritas Infoscale 8 para hosts RHEL y Oracle Linux para ayudarlo a administrar operaciones de protocolo FC, FCoE e iSCSI con LUN de ONTAP . 
---
= Configurar Veritas Infoscale 8 para FC, FCoE e iSCSI con almacenamiento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Utilice Veritas Dynamic Multipathing (VxDMP) con Veritas Infoscale 8 para administrar LUN de ONTAP .
...
----
. Verifique que los parámetros ajustables estén configurados correctamente:
+
[source, cli]
----
# vxdmpadm gettune
