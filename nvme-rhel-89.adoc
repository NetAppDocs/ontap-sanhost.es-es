...
summary: Cómo configurar el host NVMe-oF para RHEL 8,9 con ONTAP 
---
= Configurar RHEL 8.9 para NVMe-oF con almacenamiento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Aprenda a configurar hosts NVMe over Fabrics (NVMe-oF) para RHEL 8.9.  Para obtener más información sobre soporte y funciones, consultelink:hu-nvme-index.html["Descripción general de NVME-oF"^] .

*NVMe-oF con RHEL 8.9 tiene las siguientes limitaciones conocidas:*

* Actualmente no se admite el arranque SAN mediante el protocolo NVMe-oF.
* La multiruta NVMe dentro del kernel está deshabilitada de manera predeterminada en los hosts NVMe-oF en RHEL 8.9; debe habilitarla manualmente.
...
.Pasos
. Instalar RHEL 8.9 en el servidor.  Una vez completada la instalación, verifique que esté ejecutando el kernel RHEL 8.9 requerido:
+
[source, cli]
----
uname -r
...
----
. En el host RHEL 8.9, verifique la `hostnqn` cuerda en `/etc/nvme/hostnqn` :
+
[source, cli]
----
...
.FC - Marvell/QLogic
--
Configurar NVMe/FC para un adaptador Marvell/QLogic
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047["1479047"^] | Los hosts NVMe-oF de RHEL 8.9 crean controladores de descubrimiento persistente (PDC) duplicados | En los hosts NVMe-oF, puede utilizar el comando "nvme discover -p" para crear PDC.  Cuando se utiliza este comando, solo se debe crear un PDC por combinación iniciador-destino.  Sin embargo, si está ejecutando RHEL 8.9 en un host NVMe-oF, se crea un PDC duplicado cada vez que se ejecuta "nvme discover -p".  Esto genera un uso innecesario de recursos tanto en el host como en el destino. 
|===