...
== Instale las utilidades de host AIX/VIOS

Debe instalar el kit de utilidades de host AIX mientras utiliza la I/O multivía de AIX (MPIO) con almacenamiento de ONTAP.

Puede descargar el archivo comprimido que contiene los paquetes de software de Host Utilities en el link:https://mysupport.netapp.com/site/products/all/details/hostutilities/downloads-tab/download/61343/6.1/downloads["Sitio de soporte de NetApp"^]. Después de tener el archivo, debe descomprimirlo para obtener los dos paquetes de software que necesita para instalar las utilidades de host.

La versión AIX Host Utilities 6,1 corrige el problema de pérdida de memoria que se notificó en las versiones anteriores. Consulte el link:hu_aix_rn.html["notas de la versión"] para obtener información adicional.
