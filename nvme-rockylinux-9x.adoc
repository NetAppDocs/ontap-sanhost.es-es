...
summary: Cómo configurar el host NVMe-oF para Rocky Linux 9.x con ONTAP 
---
= Configurar Rocky Linux 9.x para NVMe-oF con almacenamiento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Aprenda a configurar hosts NVMe over Fabrics (NVMe-oF) para Rocky Linux 9.x. Para obtener más información sobre asistencia y funciones, consulte link:nvme-rockylinux-supported-features.html["Compatibilidad y características de Rocky Linux ONTAP"].

NVMe-oF con Rocky Linux 9.x tiene las siguientes limitaciones conocidas:
...
== Paso 2: Instale el software Rocky Linux y NVMe y verifique su configuración
...
.Pasos
. Instalar Rocky Linux 9.x en el servidor.  Una vez completada la instalación, verifique que esté ejecutando el kernel Rocky Linux 9.x requerido:
+
[source, cli]
----
uname -r
...
.NVMe/TCP
--
El protocolo NVMe/TCP no admite la operación de conexión automática.  Debe realizar manualmente las operaciones de conexión NVMe/TCP o de conexión total para descubrir los subsistemas y espacios de nombres NVMe/TCP.
...
----
=====
. Verifique que las otras combinaciones de LIF de iniciador-destino NVMe/TCP puedan obtener correctamente los datos de la página de registro de descubrimiento:
+
[source, cli]
----
nvme discover -t tcp -w host-traddr -a traddr
...
La autenticación segura en banda es compatible con NVMe/TCP entre un host Rocky Linux 9x y un controlador ONTAP .
...
----
. Genere la clave dhchap para el host Rocky Linux 9.x.
+
