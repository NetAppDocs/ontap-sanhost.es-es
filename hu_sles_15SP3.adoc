...
summary: Instale Linux Host Utilities en un SUSE Linux Enterprise Server 15 SP3 para administrar operaciones de protocolo FCP e iSCSI con LUN de ONTAP. 
---
= Configure SUSE Linux Enterprise Server 15 SP3 para FCP e iSCSI con almacenamiento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
El software de utilidades de host Linux proporciona herramientas de gestión y diagnóstico para hosts Linux que se conectan al almacenamiento de ONTAP. Al instalar las utilidades del host de Linux en un host SUSE Linux Enterprise Server 15 SP3, puede utilizar las utilidades del host para ayudarle a administrar las operaciones del protocolo FCP e iSCSI con LUN de ONTAP.
...
Puede utilizar accesos múltiples con SUSE Linux Enterprise Server 15 SP3 para gestionar los LUN de ONTAP.
