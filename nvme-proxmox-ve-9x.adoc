...
summary: Cómo configurar el host NVMe-oF para Proxmox VE 9.x con almacenamiento ONTAP 
---
= Configurar Proxmox VE 9.x para NVMe-oF con almacenamiento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
El host Proxmox VE 9.x admite los protocolos NVMe sobre canal de fibra (NVMe/FC) y NVMe sobre TCP (NVMe/TCP) con acceso asimétrico al espacio de nombres (ANA). ANA proporciona una funcionalidad de múltiples rutas equivalente al acceso a unidad lógica asimétrica (ALUA) en entornos iSCSI y FCP.

Aprenda a configurar hosts NVMe over Fabrics (NVMe-oF) para Proxmox VE 9.x. Para obtener más información sobre asistencia y funciones, consulte link:nvme-proxmox-supported-features.html["Compatibilidad y funciones de ONTAP"^].

NVMe-oF con Proxmox VE 9.x tiene la siguiente limitación conocida:
...
.Pasos
. Instale Proxmox VE 9.x en el servidor.  Una vez completada la instalación, verifique que esté ejecutando el kernel Proxmox VE 9.x requerido:
+
[source, cli]
----
uname -r
----
+
Ejemplo de versión del kernel de Proxmox VE 9.x:
...
A partir de Proxmox VE 9.0, la iopolicy predeterminada para NVMe-oF es `queue-depth`. Si quieres cambiar la iopolicy a `round-robin`, que también está disponible como opción configurable, modifica el archivo de reglas udev así:
...
----
. Modifica la regla para que `queue-depth` se cambie a `round-robin`, como se muestra en el siguiente ejemplo:
+
[listing]
----
...
----
. Verifica la iopolicy actual de tu subsistema. Reemplaza <subsystem> por el nombre de tu subsistema, por ejemplo, nvme-subsys0.
+
[source, cli]
----
...
.Mostrar un ejemplo de salida de AFF, FAS o ASA
[%collapsible]
=====
[listing, subs="+quotes"]
----
