...
* No existe compatibilidad alguna con NVMe-of. Por lo tanto, no hay compatibilidad con Linux Host Utilities para NVMe-oF en SUSE Linux Enterprise Server 15 SP3. Puede confiar en el plugin de NetApp incluido en el paquete nvme-cli nativo para NVMe-oF. Esto debería admitir todos los transportes NVMe-oF.
* Tanto el tráfico NVMe como SCSI pueden ejecutarse en el mismo host. De hecho, se espera que sea la configuración de host instalada habitualmente para los clientes. Por lo tanto, para SCSI, puede configurarse `dm-multipath` como de costumbre para LUN SCSI que causan dispositivos mpath, mientras que NVMe multipath puede utilizarse para configurar dispositivos multivía NVMe-oF en el host.
...
La opción multivía NVMe en el núcleo ya está activada de forma predeterminada en los hosts de SUSE Linux Enterprise Server, como SUSE Linux Enterprise Server 15 SP3. Por lo tanto, aquí no se requiere ningún ajuste adicional. Consulte el link:https://mysupport.netapp.com/matrix/["Herramienta de matriz de interoperabilidad"^] para obtener información precisa sobre las configuraciones compatibles.
...
. Compruebe que tiene instalados los paquetes MU de núcleo y nvme-cli necesarios en el host MU de SUSE Linux Enterprise Server 15 SP3.
+
Ejemplo:
...
Online
----
. Verifique que los puertos de iniciador NVMe/FC estén habilitados, pueda ver los puertos de destino y todos los puertos estén activos y funcionando. + En el ejemplo siguiente, solo hay un puerto de iniciador habilitado y conectado con dos LIF de destino:
+
[listing, subs="+quotes"]
----
...
ONTAP informa de UN MDT (Tamaño de transferencia MAX Data) de 8 en los datos de identificación del controlador, lo que significa que el tamaño máximo de la solicitud de E/S debe ser de hasta 1 MB. Sin embargo, para emitir solicitudes de E/S de tamaño 1MB para el host Broadcom NVMe/FC, el parámetro lpfc `lpfc_sg_seg_cnt` también debe ascender a 256 desde el valor predeterminado de 64. Utilice las siguientes instrucciones para ello:
