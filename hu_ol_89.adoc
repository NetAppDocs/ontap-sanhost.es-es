...
summary: Describe cómo usar Oracle Linux 8,9 con ONTAP 
---
= Utilice Oracle Linux 8,9 con ONTAP
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
:toclevels: 1P
...
[role="lead"]
Es posible usar las opciones de configuración del host SAN de ONTAP para configurar Oracle Linux 8,9 con ONTAP como destino.
...
Para Oracle Linux (OL) 8,9, el `/etc/multipath.conf` archivo debe existir. No es necesario realizar cambios en este archivo porque Oracle Linux 8,9 se compila con todos los ajustes necesarios para reconocer y gestionar correctamente los LUN de ONTAP.
...
El sistema operativo Oracle Linux 8,9 se compila para reconocer los LUN de ONTAP y establecer automáticamente los parámetros de configuración correctos para las configuraciones ASA y no ASA. Puede optimizar aún más el rendimiento de la configuración del host con los siguientes ajustes recomendados.
...
NOTE: Para configurar Oracle Linux 8,9 RedHat Enterprise Kernel, utilice link:hu_rhel_89.html#recommended-settings["configuración recomendada"]para Red Hat Enterprise Linux (RHEL) 8,9.



== Configuración de KVM

También puede utilizar la configuración recomendada para configurar una máquina virtual basada en kernel (KVM). No se necesitan cambios para configurar un KVM, ya que la LUN está asignada al hipervisor.



== Problemas conocidos

No existen problemas conocidos para la versión Oracle Linux 8,9 con ONTAP.
