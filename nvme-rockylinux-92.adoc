...
---
= Configurar Rocky Linux 9.2 para NVMe-oF con almacenamiento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Aprenda a configurar hosts NVMe over Fabrics (NVMe-oF) para Rocky Linux 9.2. Para obtener más información sobre asistencia y funciones, consulte link:nvme-rockylinux-supported-features.html["Compatibilidad y características de Rocky Linux ONTAP"].

*NVMe-oF con Rocky Linux 9.2 tiene las siguientes limitaciones conocidas:*
...
.Pasos
. Instalar Rocky Linux 9.2 en el servidor.  Una vez completada la instalación, verifique que esté ejecutando el kernel Rocky Linux 9.2 requerido:
+
[source, cli]
----
uname -r
