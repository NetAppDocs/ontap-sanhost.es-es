...
summary: Instale las utilidades de host de Linux en SUSE Linux Enterprise Server 15 SP7 para gestionar las operaciones de protocolo FCP e iSCSI con LUN de ONTAP. 
---
= Configurar SUSE Linux Enterprise Server 15 SPx para FCP e iSCSI con almacenamiento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
El software Linux Host Utilities proporciona herramientas de administración y diagnóstico para los hosts Linux que están conectados al almacenamiento ONTAP . Cuando instale las utilidades de host de Linux en un host SUSE Linux Enterprise Server 15 SPx, podrá utilizarlas para ayudarle a gestionar las operaciones de los protocolos FCP e iSCSI con LUN de ONTAP .
...
Puede utilizar rutas múltiples con SUSE Linux Enterprise Server 15 SPx para administrar LUN de ONTAP .
...
SUSE Linux Enterprise Server 15 SPx con almacenamiento ONTAP tiene los siguientes problemas conocidos.
...
| link:https://mysupport.netapp.com/NOW/cgi-bin/bol?Type=Detail&Display=1154309["1154309"^] | Es posible que el host SLES 15 con más de 20 LUN asignadas entre en modo de mantenimiento después de un reinicio | Es posible que el host SLES 15 con más de 20 LUN asignadas entre en modo de mantenimiento después de un reinicio. El modo de mantenimiento se convierte en modo de usuario único después del mensaje:
