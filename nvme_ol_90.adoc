...
[role="lead"]
NVMe over Fabrics (NVMe-oF), incluidos NVMe/FC y otros transportes, es compatible con Oracle Linux 9,0 con acceso al espacio de nombres asimétrico (ANA). En entornos de NVMe-oF, ANA es el equivalente a la multivía ALUA en entornos iSCSI y FC y se implementa con NVMe multivía en kernel.
...
.Pasos
. Instale Oracle Linux 9,0 GA en el servidor. Una vez finalizada la instalación, compruebe que está ejecutando el núcleo GA de Oracle Linux 9,0 especificado.
+
[listing]
----
# uname -r
...
.Pasos
. Verifique la siguiente configuración de NVMe/FC en el host Oracle Linux 9,0:
+
[listing]
----
...
Si tiene algún problema con la configuración, el registro detallado puede proporcionar información valiosa esencial para la solución de problemas.
