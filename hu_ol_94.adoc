...
summary: Describe cómo usar Oracle Linux 9,4 con ONTAP 
---
= Utilice Oracle Linux 9,4 con ONTAP
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
:toclevels: 1P
...
[role="lead"]
Es posible usar las opciones de configuración del host SAN de ONTAP para configurar Oracle Linux 9,4 con ONTAP como destino.
...
Puede utilizar el acceso multivía con Oracle Linux 9,4 para gestionar los LUN de ONTAP. Para utilizar la función multivía, `/etc/multipath.conf` el archivo debe existir. Si el `/etc/multipath.conf` archivo no existe, puede crear un archivo vacío de cero bytes mediante el comando táctil `/etc/multipath.conf`. No es necesario realizar cambios en `/etc/multipath.conf` el archivo porque Oracle Linux 9,4 se compila con todos los ajustes necesarios para reconocer y gestionar correctamente los LUN de ONTAP.
...
El sistema operativo Oracle Linux 9,4 reconoce los LUN de ONTAP y define automáticamente todos los parámetros de configuración correctamente para configuraciones ASA y no ASA. Puede optimizar aún más el rendimiento de la configuración del host con los siguientes ajustes recomendados.
...
NOTE: Para configurar el kernel compatible con Red Hat (RHCK) de Oracle Linux 9,4, utilice para link:hu_rhel_94.html#recommended-settings["configuración recomendada"]Red Hat Enterprise Linux (RHEL) 9,4.
...
No existen problemas conocidos en la configuración del host FC para OL 9,4 con la versión ONTAP.
