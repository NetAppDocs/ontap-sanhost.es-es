...
summary: Cómo configurar el host NVMe-oF para Rocky Linux 9.4 con ONTAP 
---
= Configurar Rocky Linux 9.4 con NVMe-oF para almacenamiento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Los hosts Rocky Linux 9.4 admiten los protocolos NVMe/FC y NVMe/TCP con acceso asimétrico al espacio de nombres (ANA). ANA es equivalente al acceso asimétrico a unidades lógicas (ALUA) multipath en entornos iSCSI y FCP y se implementa mediante la función de multipath NVMe integrada en el kernel.
...
Puede utilizar el siguiente soporte y funciones con la configuración de host NVMe-oF para Rocky Linux 9.4. También debe revisar las limitaciones conocidas antes de iniciar el proceso de configuración.
...
+
** Rocky Linux 9.4 habilita la multiruta NVMe dentro del kernel para espacios de nombres NVMe de manera predeterminada, eliminando la necesidad de configuraciones explícitas.
...
Utilice el siguiente procedimiento para validar las versiones mínimas compatibles del software Rocky Linux 9.4.

.Pasos
. Instale Rocky Linux 9.4 en el servidor. Una vez completada la instalación, verifique que esté ejecutando el kernel de Rocky Linux 9.4 especificado:
+
[source, cli]
----
uname -r
