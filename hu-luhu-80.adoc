...
Cuando instalas las utilidades de host de Linux, no se cambia ninguna configuración ni el comportamiento de tu host.
...
NOTE: Las utilidades de host de Linux tienen una dependencia de instalación con la biblioteca libHBAAPI.so. Necesitas instalar uno de los paquetes mencionados arriba, incluso si un host está destinado solo a una configuración iSCSI. O también puedes usar herramientas a nivel del sistema operativo para la gestión de LUN y rutas.
