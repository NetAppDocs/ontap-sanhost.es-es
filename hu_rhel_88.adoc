...
summary: Describe cómo utilizar Red Hat Enterprise Linux 8,8 con ONTAP 
---
= Utilice Red Hat Enterprise Linux 8,8 con ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Puede utilizar los ajustes de configuración del host SAN de ONTAP para configurar Red Hat Enterprise Linux 8,8 con ONTAP como destino.
...
Para Red Hat Enterprise Linux (RHEL) 8,8, el `/etc/multipath.conf` el archivo debe existir, pero no es necesario realizar cambios específicos en el archivo. RHEL 8,8 se compila con todas las opciones de configuración necesarias para reconocer y gestionar correctamente las LUN de ONTAP.
...
El sistema operativo RHEL 8,8 se compila para reconocer los LUN de ONTAP y establecer automáticamente todos los parámetros de configuración correctamente para configuraciones de ASA y que no son ASA. Puede optimizar aún más el rendimiento de la configuración del host con los siguientes ajustes recomendados.
...
No existen problemas conocidos para la versión de RHEL 8,8 con ONTAP.
