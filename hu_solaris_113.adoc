...
A continuación se muestran algunos valores de parámetros recomendados para Solaris 11,3 SPARC y x86_64 con LUN de ONTAP. Estos valores de parámetros los establece Host Utilities.
...
* Por lo general, NetApp admite el uso de estas tecnologías de virtualización donde Oracle admite la configuración general y cualquier partición con acceso directo a LUN aparece en la https://imt.netapp.com/matrix/#welcome["Herramienta de matriz de interoperabilidad"^] en una configuración admitida. Esto incluye contenedores raíz, dominios de I/O LDOM y LDOM que usan NPIV para acceder a las LUN.
* Las particiones o máquinas virtuales que utilizan sólo recursos de almacenamiento virtualizado, como A `vdsk`, no necesitan cualificación específica ya que no tienen acceso directo a las LUN de NetApp. Sólo se debe encontrar la partición/equipo virtual que tiene acceso directo a la LUN subyacente, como un dominio de E/S de LDOM, en la https://imt.netapp.com/matrix/#welcome["Herramienta de matriz de interoperabilidad"^].
