...
----
+
El siguiente ejemplo muestra una `libnvme` versión de paquete:
...
.Cambios en iopolicy para las versiones RHEL 9.x
* A partir de RHEL 9.7, la iopolicy por defecto es queue-depth.
* En RHEL 9.6, round-robin es la iopolicy por defecto, pero queue-depth está disponible como opción configurable.
* Antes de RHEL 9.6, round-robin es la única iopolicy NVMe-oF soportada.


Si estás usando RHEL 9.6 y quieres cambiar la iopolicy a `queue-depth`, modifica el archivo de reglas udev de la siguiente manera:
