...
summary: Cómo configurar el host NVMe-oF para RHEL 8x con ONTAP 
---
= Configurar RHEL 8x para NVMe-oF con almacenamiento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Aprenda a configurar hosts NVMe over Fabrics (NVMe-oF) para RHEL 8x. Para obtener más información sobre asistencia y funciones, consulte link:nvme-rhel-supported-features.html["Compatibilidad y características de RHEL ONTAP"].

NVMe-oF con RHEL 8x tiene las siguientes limitaciones conocidas:

* Actualmente no se admite el arranque SAN mediante el protocolo NVMe-oF.
* La multiruta NVMe dentro del kernel está deshabilitada de manera predeterminada en los hosts NVMe-oF en RHEL 8x; debe habilitarla manualmente.
...
.Pasos
. Instalar RHEL 8x en el servidor.  Una vez completada la instalación, verifique que esté ejecutando el kernel RHEL 8x requerido:
+
[source, cli]
----
uname -r
...
----
. En el host RHEL 8x, verifique la `hostnqn` cuerda en `/etc/nvme/hostnqn` :
+
[source, cli]
----
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047["1479047"^] | Los hosts RHEL 8x NVMe-oF crean controladores de descubrimiento persistentes (PDC) duplicados | En los hosts NVMe-oF, puede utilizar el comando "nvme discover -p" para crear PDC.  Cuando se utiliza este comando, solo se debe crear un PDC por combinación iniciador-destino.  Sin embargo, si está ejecutando RHEL 8x en un host NVMe-oF, se crea un PDC duplicado cada vez que se ejecuta "nvme discover -p".  Esto genera un uso innecesario de recursos tanto en el host como en el destino. 
|===