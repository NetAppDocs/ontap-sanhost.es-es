...
---
= Configuración del host NVMe-of para RHEL 8.5 con ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
RHEL 8.5 admite NVMe over Fabrics o NVMe-of (incluidos NVMe/FC y otros transportes) con ANA (acceso de espacio de nombres asimétrico). ANA es el equivalente ALUA en el entorno NVMe-of y actualmente se implementa con NVMe multivía en el kernel. Los detalles para habilitar NVMe-of con NVMe multivía en el kernel mediante ANA en RHEL 8.5 y ONTAP, ya que el destino se ha documentado aquí.
...
* NVMe/TCP en RHEL 8.5 sigue siendo una función de vista previa de la tecnología debido a problemas abiertos. Consulte la https://["Notas de la versión de RHEL 8.5"^] para obtener más detalles.
...
La bandeja de entrada nativa `qla2xxx` El controlador incluido en el kernel RHEL 8.5 GA tiene las últimas correcciones previas que son esenciales para la compatibilidad con ONTAP.
