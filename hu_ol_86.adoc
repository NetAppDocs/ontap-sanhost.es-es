...
summary: Describe cómo usar Oracle Linux 8.6 con ONTAP 
---
= Uso de Oracle Linux 8.6 con NetApp ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
...
Para Oracle Linux (OL) 8.6 la `/etc/multipath.conf` el archivo debe existir, pero no es necesario realizar cambios específicos en el archivo. OL 8.6 se compila con todas las configuraciones necesarias para reconocer y gestionar correctamente las LUN de ONTAP.
...
NOTE: Para configurar Oracle Linux 8.6 RedHat Enterprise Kernel (RHCK), utilice link:hu_rhel_86.html#recommended-settings["configuración recomendada"] Para Red Hat Enterprise Linux (RHEL) 8.6.
...
No existen problemas conocidos para Oracle Linux 8.6.


NOTE: Para ver los problemas conocidos de Oracle Linux (kernel compatible con Red Hat), consulte link:hu_rhel_86.html#known-problems-and-limitations["problemas conocidos"] En la documentación correspondiente de la versión de Red Hat Enterprise Linux.
