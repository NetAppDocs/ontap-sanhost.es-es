...
Aprenda a configurar hosts NVMe over Fabrics (NVMe-oF) para Rocky Linux 9.6. Para obtener más información sobre asistencia y funciones, consulte link:nvme-rockylinux-supported-features.html["Compatibilidad y características de Rocky Linux ONTAP"].

*NVMe-oF con Rocky Linux 9.6 tiene las siguientes limitaciones conocidas:*
...
.Pasos
. Instalar Rocky Linux 9.6 en el servidor.  Una vez completada la instalación, verifique que esté ejecutando el kernel Rocky Linux 9.6 requerido:
+
[source, cli]
----
uname -r
