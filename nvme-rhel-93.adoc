...
Aprenda a configurar hosts NVMe over Fabrics (NVMe-oF) para RHEL 9.3.  Para obtener más información sobre soporte y funciones, consultelink:hu-nvme-index.html["Descripción general de NVME-oF"^] .
...
.Pasos
. Instalar RHEL 9.3 en el servidor.  Una vez completada la instalación, verifique que esté ejecutando el kernel RHEL 9.3 requerido:
+
[source, cli]
----
uname -r
...
cat /etc/nvme/hostnqn
----
. Genere la clave dhchap para el host RHEL 9.3.
+
