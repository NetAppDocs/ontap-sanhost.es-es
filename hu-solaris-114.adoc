...
** NetApp admite el uso de tecnologías de virtualización de Solaris donde la configuración general sea compatible con Oracle y cualquier partición con acceso directo a LUNs esté listada en la link:https://imt.netapp.com/matrix/#welcome["IMT"^] en una configuración compatible. Esto incluye contenedores raíz, dominios de E/S LDOM y LDOM que usan NPIV para acceder a LUNs.
