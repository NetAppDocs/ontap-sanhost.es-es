...
Si tu host está conectado a LUNs de otros proveedores y se ha anulado alguna de las configuraciones de los parámetros de multivía, debes corregirlas añadiendo estrofas en el archivo  `multipath.conf` que se apliquen específicamente a los LUNs de ONTAP. Desplázate hacia abajo en el archivo para añadir estas estrofas. Si no corriges los parámetros, es posible que los LUNs de ONTAP no funcionen como esperas.
