...
---
= Configure los ajustes del Registro para las utilidades del host de Windows
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
[NOTE]
====
Necesita cambiar el valor del parámetro LimTransferSize después de instalar Windows Host Utilities 7,2, consulte <<nvme_parameters,Parámetros de NVMe>>.

Para todos los demás parámetros y claves del Registro de Windows Host Utilities 7,2 y 7,1, no debe cambiar los valores a menos que el soporte técnico le indique que lo haga.
