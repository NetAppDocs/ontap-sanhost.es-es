...
---
= Configurar Rocky Linux 9.5 para NVMe-oF con almacenamiento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Aprenda a configurar hosts NVMe over Fabrics (NVMe-oF) para Rocky Linux 9.5. Para obtener más información sobre asistencia y funciones, consulte link:nvme-rockylinux-supported-features.html["Compatibilidad y características de Rocky Linux ONTAP"].

NVMe-oF con Rocky Linux 9.5 no tiene limitaciones conocidas.
...
.Pasos
. Instalar Rocky Linux 9.5 en el servidor.  Una vez completada la instalación, verifique que esté ejecutando el kernel Rocky Linux 9.5 requerido:
+
[source, cli]
----
uname -r
