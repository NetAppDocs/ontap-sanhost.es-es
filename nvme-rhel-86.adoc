...
---
= Configurar RHEL 8.6 para NVMe-oF con almacenamiento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Aprenda a configurar hosts NVMe over Fabrics (NVMe-oF) para RHEL 8.6.  Para obtener más información sobre soporte y funciones, consultelink:hu-nvme-index.html["Descripción general de NVME-oF"^] .

*NVMe-oF con RHEL 8.6 tiene las siguientes limitaciones conocidas:*

* Actualmente no se admite el arranque SAN mediante el protocolo NVMe-oF.
* La multiruta NVMe dentro del kernel está deshabilitada de manera predeterminada en los hosts NVMe-oF en RHEL 8.6; debe habilitarla manualmente.
...
.Pasos
. Instalar RHEL 8.6 en el servidor.  Una vez completada la instalación, verifique que esté ejecutando el kernel RHEL 8.6 requerido:
+
[source, cli]
----
uname -r
...
----
. En el host RHEL 8.6, verifique la `hostnqn` cuerda en `/etc/nvme/hostnqn` :
+
[source, cli]
----
...
NOTE: Si el `hostnqn` Las cadenas no coinciden, debes utilizar el `vserver modify` comando para actualizar el `hostnqn` cadena en el subsistema NVMe del sistema de almacenamiento ONTAP correspondiente para que coincida con `hostnqn` cadena `/etc/nvme/hostnqn` en el host.
