...
---
= Uso de Oracle Linux 7.0 con NetApp ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
.Lo que necesitará
Si decide utilizar el arranque SAN, debe ser compatible con su configuración. Puede utilizar el https://["Herramienta de matriz de interoperabilidad de NetApp"^] Para verificar si el sistema operativo, el HBA, el firmware del HBA y la BIOS de arranque del HBA y las versiones de ONTAP son compatibles.
...
NOTE: Para configurar Oracle Linux 7.0 RedHat Enterprise Kernel (RHCK), utilice link:hu_rhel_70.html#recommended-settings["configuración recomendada"] Para Red Hat Enterprise Linux (RHEL) 7.0.
