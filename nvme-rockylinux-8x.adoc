...
summary: Cómo configurar el host NVMe-oF para Rocky Linux 8.x con ONTAP 
---
= Configurar Rocky Linux 8.x para NVMe-oF con almacenamiento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Los hosts Rocky Linux admiten los protocolos NVMe sobre canal de fibra (NVMe/FC) y NVMe sobre TCP (NVMe/TCP) con acceso asimétrico al espacio de nombres (ANA). ANA proporciona una funcionalidad de múltiples rutas equivalente al acceso a unidad lógica asimétrica (ALUA) en entornos iSCSI y FCP.

Aprenda a configurar hosts NVMe over Fabrics (NVMe-oF) para Rocky Linux 8.x. Para obtener más información sobre asistencia y funciones, consulte link:nvme-rockylinux-supported-features.html["Compatibilidad y características de Rocky Linux ONTAP"].

NVMe-oF con Rocky Linux 8.x tiene las siguientes limitaciones conocidas:

* Actualmente no se admite el arranque SAN mediante el protocolo NVMe-oF.
* La multiruta NVMe dentro del kernel está deshabilitada de manera predeterminada en los hosts NVMe-oF en Rocky Linux 8.x; debe habilitarla manualmente.
* NVMe/TCP está disponible como una versión preliminar de tecnología debido a problemas conocidos.




== Paso 1: Instale el software Rocky Linux y NVMe y verifique su configuración
...
.Pasos
. Instalar Rocky Linux 8.x en el servidor.  Una vez completada la instalación, verifique que esté ejecutando el kernel Rocky Linux 8.x requerido:
+
[source, cli]
----
uname -r
...
Directiva sin resolver en <stdin> - include::_include/nvme/linux-validate-software-version-rockylinux.adoc[]
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047["1479047"^] | Los hosts NVMe-oF de Rocky Linux 8.x crean controladores de descubrimiento persistentes duplicados | En los hosts NVMe-oF, puede usar el comando "nvme discover -p" para crear controladores de descubrimiento persistentes (PDC).  Sin embargo, si está ejecutando Rocky Linux 8.x en un host NVMe-oF, se crea un PDC duplicado cada vez que se ejecuta "nvme discover -p". Cuando se utiliza este comando, solo se debe crear un PDC por combinación iniciador-destino. Sin embargo, si está ejecutando Rocky Linux 8.x en un host NVMe-oF, se crea un PDC duplicado cada vez que se ejecuta "nvme discover -p". Esto genera un uso innecesario de recursos tanto en el host como en el destino. 
|===