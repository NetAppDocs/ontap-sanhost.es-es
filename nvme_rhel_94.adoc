...
NVMe/TCP no tiene una funcionalidad de conexión automática. Por lo tanto, necesitamos ejecutar la funcionalidad de conexión o conexión NVMe/TCP manualmente para detectar los subsistemas y los espacios de nombres NVMe/TCP.
...
NOTE: A partir de RHEL 9,4, la configuración predeterminada para NVMe/TCP `ctrl_loss_tmo` timeout desactivado, lo que significa que no hay límite en el número de reintentos (reintentos indefinidos). Por lo tanto, no es necesario configurar manualmente un determinado `ctrl_loss_tmo` duración de tiempo de espera cuando se utilizan los comandos nvme connect o nvme connect-all (opción -l ). Con este comportamiento predeterminado, las controladoras NVMe/TCP no experimentarán tiempos de espera en caso de un fallo de ruta y permanecerán conectadas de forma indefinida.
