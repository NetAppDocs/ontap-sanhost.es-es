...
summary: Describe cómo usar Oracle Linux 9,1 con ONTAP 
---
= Utilice Oracle Linux 9,1 con ONTAP
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
:toclevels: 1P
...
[role="lead"]
Es posible usar las opciones de configuración del host SAN de ONTAP para configurar Oracle Linux 9,1 con ONTAP como destino.
...
Para OL 9,1 el `/etc/multipath.conf` el archivo debe existir, pero no es necesario realizar cambios específicos en el archivo. OL 9,1 se compila con todos los ajustes necesarios para reconocer y gestionar correctamente las LUN de ONTAP.
...
El sistema operativo Oracle Linux 9,1 se compila para reconocer los LUN de ONTAP y definir automáticamente todos los parámetros de configuración correctamente para configuraciones ASA y no ASA. Puede optimizar aún más el rendimiento de la configuración del host con los siguientes ajustes recomendados.
...
NOTE: Para configurar Oracle Linux 9,1 Red Hat Enterprise Kernel (RHCK), utilice para link:hu_rhel_91.html#recommended-settings["configuración recomendada"]Red Hat Enterprise Linux (RHEL) 9,1.
...
La versión Oracle Linux 9,1 con NetApp ONTAP tiene los siguientes problemas conocidos:
