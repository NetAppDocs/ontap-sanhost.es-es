...
---
= Uso de Oracle Linux 9,1 con NetApp ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
...
Para OL 9,1 el `/etc/multipath.conf` el archivo debe existir, pero no es necesario realizar cambios específicos en el archivo. OL 9,1 se compila con todas las configuraciones necesarias para reconocer y gestionar correctamente las LUN de ONTAP.
...
El sistema operativo Oracle Linux 9,1 se compila para reconocer los LUN de ONTAP y definir automáticamente todos los parámetros de configuración correctamente para la configuración ASA y no ASA. Puede optimizar aún más el rendimiento de la configuración del host con los siguientes ajustes recomendados.
...
Oracle Linux 9,1 con la versión NetApp ONTAP tiene los siguientes problemas conocidos:
...
| https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1508554["1508554"^] | La utilidad de LUN de SAN con HBA de Emulex necesita enlaces simbólicos de paquetes de la biblioteca  a| 
