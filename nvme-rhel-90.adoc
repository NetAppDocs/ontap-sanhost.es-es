...
---
= Configurar RHEL 9.0 para NVMe-oF con almacenamiento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Aprenda a configurar hosts NVMe over Fabrics (NVMe-oF) para RHEL 9.0.  Para obtener más información sobre soporte y funciones, consultelink:hu-nvme-index.html["Descripción general de NVME-oF"^] .

*NVMe-oF con RHEL 9.0 tiene las siguientes limitaciones conocidas:*
...
.Pasos
. Instalar RHEL 9.0 en el servidor.  Una vez completada la instalación, verifique que esté ejecutando el kernel RHEL 9.0 requerido:
+
[source, cli]
----
uname -r
...
----
. En el host RHEL 9.0, verifique la `hostnqn` cuerda en `/etc/nvme/hostnqn` :
+
[source, cli]
----
