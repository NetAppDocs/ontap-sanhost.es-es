...
summary: Describe cómo configurar NVMe/FC para SUSE Linux Enterprise Server 15 SP4 con ONTAP 
---
= Configuración de host de NVMe-of para SUSE Linux Enterprise Server 15 SP4 con ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
[role="lead"]
SUSE Linux Enterprise Server 15 SP4 (SLES15 SP4) con ANA (acceso de espacio de nombres asimétrico) admite NVMe over Fabrics o NVMe-of (incluidos NVMe/FC y otros transportes). ANA es el equivalente ALUA en el entorno NVMe-of y actualmente se implementa con NVMe multivía en el kernel. Los detalles para habilitar NVMe-of con NVMe multivía en el kernel mediante ANA en SLES15 SP4 y ONTAP a medida que se ha documentado el destino aquí.
...
* SLES15 SP4 es compatible con NVMe/FC y otros transportes.
* No existe `sanlun` Compatibilidad con NVMe-of. Por lo tanto, no existe compatibilidad con LUHU para NVMe-of en SLES15 SP4. Puede confiar en el complemento de NetApp que se incluye en la configuración nativa `nvme-cli` en lugar de eso, lo mismo. Esto debería funcionar para todos los transportes NVMe-of.
* Tanto el tráfico NVMe como SCSI pueden ejecutarse en el mismo host coexistente. De hecho, se espera que sea la configuración de host instalada habitualmente. Por lo tanto, para SCSI, puede configurar `dm-multipath` Como es habitual para los LUN de SCSI, que dan como resultado dispositivos mpath, mientras que es posible que se utilice NVMe multivía para configurar los dispositivos multivía en el host.
...
NVMe multivía en el kernel ya está habilitado de forma predeterminada en hosts SLES como SLES15 SP4. Por lo tanto, aquí no se requiere ningún ajuste adicional. Consulte la link:https://mysupport.netapp.com/matrix/["Matriz de interoperabilidad de NetApp"^] para obtener información precisa sobre las configuraciones compatibles.
...
* Compruebe que dispone del kernel y. `nvme-cli` Paquetes de actualización de mantenimiento (UM) instalados en el host de UM SLES15 SP4.
+
Ejemplo:
...
* Compruebe la `hostnqn` cadena en  `/etc/nvme/hostnqn` en el host y asegúrese de que coincide correctamente con `hostnqn` Cadena para el subsistema correspondiente en la matriz ONTAP. Por ejemplo:
+
[listing]
----
...
Continúe con los pasos siguientes según el adaptador de FC que se esté usando en el host.
...
----
+
Los controladores lpfc más nuevos (tanto la bandeja de entrada como la bandeja de salida) ya lo tienen `lpfc_enable_fc4_type` por lo tanto, el valor predeterminado se establece en 3, ya no es necesario estableclo explícitamente en la `/etc/modprobe.d/lpfc.conf`, y recrear el `initrd`. La `lpfc nvme` la compatibilidad ya está habilitada de forma predeterminada:
...
----
+
El controlador lpfc de la bandeja de entrada nativa existente ya es más reciente y compatible con NVMe/FC. Por lo tanto, no es necesario instalar el controlador lpfc fuera de la caja (OOB).
...
Online
----
. Verifique que los puertos iniciadores NVMe/FC estén habilitados y que pueda ver los puertos objetivo y que todos estén en funcionamiento. En este ejemplo, solo un puerto del iniciador está habilitado y conectado con dos LIF de destino como se ve en el resultado:
+
[listing]
----
...
----
. Ejecute un comando dracut -f y reinicie el host.
...
El controlador de bandeja de entrada nativa qla2xxx incluido en el nuevo kernel de UM SLES15 SP4 cuenta con las últimas correcciones previas, esenciales para la compatibilidad con ONTAP.
...
* No se puede reconectar automáticamente después de que las rutas se restablezcan NVMe/TCP no pueden volver a conectarse automáticamente a una ruta que se restablezca más allá de la predeterminada `ctrl-loss-tmo` temporizador de 10 minutos después de una ruta hacia abajo.
...
----
. Ejecución `nvme connect-all` Command entre todas las LIF de iniciador NVMe/TCP admitidas en los nodos. Asegúrese de establecer un valor más largo `ctrl_loss_tmo` período de reintento del temporizador (por ejemplo, 30 minutos, que se puede ajustar a través de -l 1800) durante el `connect-all` de modo que se reintente durante un período de tiempo más largo en caso de una pérdida de ruta. Por ejemplo:
+
[listing]
----
...
Y
----
. Compruebe que la configuración NVMe-of adecuada (como, el modelo establecido en la controladora ONTAP de NetApp y el equilibrio de carga que se incluye en la operación por turnos) para los espacios de nombres ONTAP respectivos se reflejen correctamente en el host:
+
[listing]
----
