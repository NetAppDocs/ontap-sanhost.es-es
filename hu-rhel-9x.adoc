...
summary: Instale las utilidades de host de Linux en un host RHEL 9.x para administrar las operaciones de protocolo FCP e iSCSI con LUN de ONTAP . 
---
= Configurar RHEL 9.x para FCP e iSCSI con almacenamiento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
El software Linux Host Utilities proporciona herramientas de administración y diagnóstico para los hosts Linux que están conectados al almacenamiento ONTAP . Cuando instale las utilidades de host de Linux en un host Red Hat Enterprise Linux (RHEL) 9.x, podrá utilizarlas para ayudarle a gestionar las operaciones de los protocolos FCP e iSCSI con LUN de ONTAP .
...
Puedes utilizar multipathing con RHEL 9.x para gestionar LUN de ONTAP .
...
RHEL 9.x con almacenamiento ONTAP tiene los siguientes problemas conocidos.
