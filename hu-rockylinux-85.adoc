...
summary: Instale Linux Host Utilities en un host Rocky Linux 8.5 para administrar operaciones de protocolo FCP e iSCSI con LUN de ONTAP. 
---
= Configurar Rocky Linux 8.5 para FCP e iSCSI con almacenamiento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
El software de utilidades de host Linux proporciona herramientas de gestión y diagnóstico para hosts Linux que se conectan al almacenamiento de ONTAP. Al instalar Linux Host Utilities en un host Rocky Linux 8.5, puede usar las Host Utilities para ayudarlo a administrar las operaciones de protocolo FCP e iSCSI con LUN de ONTAP.
...
Puede utilizar rutas múltiples con Rocky Linux 8.5 para administrar LUN de ONTAP.
